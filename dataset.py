...
# Extract the magnitude and phase components of the STFT
magnitude = np.abs(stft)
phase = np.angle(stft)

demodulated_phase = np.unwrap(phase)

real = np.sin(demodulated_phase)
imag = np.cos(demodulated_phase)

#r = np.arcsin(real_part)
#i = np.arccos(imag_part)
s = np.arctan2(real, imag)


#s = np.arctan2(np.arccos(real_part), np.arcsin(imag_part))

# Reconstruct the signal from the demodulated phase and magnitude
complex_spectrogram = magnitude * np.exp(1j * s)
y = librosa.istft(complex_spectrogram, hop_length=128)
#y = y * -1.

sf.write('test1.wav', signal.flatten(), sr)
sf.write('test2.wav', y.flatten(), sr)


ipd.display(ipd.Audio('test1.wav'))
ipd.display(ipd.Audio('test2.wav'))
